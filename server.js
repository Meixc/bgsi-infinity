...
app.get('/api/roblox-pfp/:userId', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ error: "Avatar error" }); }
});
...
app.post('/api/verify-bio', async (req, res) => {
    ...
    try {
        ...
        } else { res.json({ success: false, message: "Code not found!" }); }
...
app.get('/api/leaderboard', (req, res) => {
    const lb = Object.values(userDB).sort((a,b) => b.wagered - a.wagered).slice(0,10).map((u,i) => ({ rank: i+1, ...u }));
    res.json(lb);
});
...
io.on('connection', (socket) => {
    ...
    socket.on('cancel_flip', (id) => {
        const idx = activeFlips.findIndex(g => g.id === id);
